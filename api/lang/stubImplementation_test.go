...
func TestGenerateConceptShouldReturnFileDiff(t *testing.T) {
	...
	var want lsp.WorkspaceEdit
	want.Changes = make(map[string][]lsp.TextEdit, 0)
	uri := util.ConvertPathToURI(lsp.DocumentURI(filepath.Join(testData, "concept1.cpt")))
	textEdit := lsp.TextEdit{
		NewText: "# foo bar\n* ",
		Range: lsp.Range{
...
func TestGenerateConceptInExisitingFile(t *testing.T) {
	...
	uri := string(util.ConvertPathToURI(lsp.DocumentURI(cptFile)))
...
func TestGenerateConceptInNewFileWhenDefaultExisits(t *testing.T) {
	...
	uri := util.ConvertPathToURI(lsp.DocumentURI(filepath.Join(testData, "concept2.cpt")))
